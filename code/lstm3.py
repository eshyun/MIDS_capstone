...
sys.path.append(f"{os.getenv('HOME')}/projects/trading/aegis")
import stock
...
def set_up_data(companies, nlp_dir, revenue_dir, dest_dir, n_lags, n_forecast):
	# companies : list like names or codes of companies

	datasets = {}
	orig_dfs = {}
	n_features = 0
	for company in sorted(companies):
		ticker = stock.get_code(company)
		print(ticker)
		
		# read data from sqlite3 database not from csv
		# df = read_csv(company, header=0, parse_dates=[0], index_col=0, squeeze=True) #, date_parser=parser)
		# Only after 2015
		# df = df[df.index > "2015-01-01"]

		df = stock.DataReader(company, start='2010-01-01')
		print(df.head())

		# rename 'close' to 'Adj Close' to make life easier^^
		df.rename(columns ={'close': 'Adj Close'}, inplace=True)

		# Move 'Adj Close' (predicted column) to last column
		dataset = df[['volume', 'Adj Close']]
		print('%s has %s rows' % (company, len(dataset)))
		print(dataset.head())
		...
		datasets[ticker] = reframed.values
